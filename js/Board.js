class Board {
  ...
  clickHandler(item) {
    ...
    if (this.cells.cell.isMove(item.position, emptyPosition)) {
      let savePosition = { x: item.position.x, y: item.position.y, left: item.position.left, top: item.position.top }
      this.cells.cell.translate(item.cell, emptyPosition.left, emptyPosition.top)
      this.cells.cell.translate(emptyCell, item.position.left, item.position.top)
      item.position = { x: emptyPosition.x, y: emptyPosition.y, left: emptyPosition.left, top: emptyPosition.top }
      emptyObjectCell.position = { x: savePosition.x, y: savePosition.y, left: savePosition.left, top: savePosition.top }
    }
  ...
  isWin() {
    for (let i = 0; i < this.staticCells.length; i++) {
      ...
      if (position.x !== winPosition.x || position.y !== winPosition.y) return false
